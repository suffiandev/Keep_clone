...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['archive_note'])) {
    ...
    // Update the note's archive status in the database
    ...
    if (isset($_GET['view']) && $_GET['view'] === 'archive') {
        ...
        if ($resultAllArchived->num_rows > 0) {
            ...
            foreach ($allArchivedNotes as $note) {
                ...
                echo '<button type="submit" name="archive_note" onmouseover="showMessage(\'hoverArchiveMessage_' . $note['id'] . '\')" onmouseout="hideMessage(\'hoverArchiveMessage_' . $note['id'] . '\')">';
                ...
                echo '</svg>';
                echo '</button>';
                echo '<div id="hoverArchiveMessage_' . $note['id'] . '">Archive</div>';
                echo '</form>';
